...
async def login(form_data: OAuth2PasswordRequestForm = Depends()):
  ...
  (admin_email, admin_password) = (
    settings.ADMIN_EMAIL, settings.ADMIN_HASHED_PASS)
  # verify email
  ...
  # return tikens
  return dict(
    access_token=token_maker.create_access_token(username),
    refresh_token=token_maker.create_refresh_token(username),
  )
